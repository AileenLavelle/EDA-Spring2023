...
#ggplot map
ggplot() +
  geom_sf(dat=huc8_sf, aes(fill=ACRES))+
...
#Create a map
mapview(huc8_sf, col.regions ="cornflowerblue") +
  mapview(upperNeuse_sf, col.regions = "darkorchid")
...
unique(huc8s_WGS84$DWQ_Basin) 

#Summarize on DWQ Basin value
summary(huc8s_WGS84$DWQ_Basin)

#Map the data
huc2_sf <- huc8s_WGS84 %>%
  summarise(ACRES_Sum = sum(ACRES)) %>%
  summarise(SQMiles_Sum = sum(SQ_MILES))      
...
huc2_sf <- st_read('./Data/Spatial/NCHUC8.shp')
...
mapview(the_huc_clip) + mapview(the_triangle, alpha.regions = 0, color = 'yellow')
...
#can also use st_filter instead of st_intersect
