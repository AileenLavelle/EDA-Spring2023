...
#Create a plot
#paste command brings together strings
#paste 0 does not include spaces
#expression command makes math mathematical formulas
Plot_text <-
...
#Save the plot
ggsave(
  filename = "./Data/Raw/Plot_text.jpg",
  plot = Plot_text, 
  height = 4, 
  width = 6, 
  units = "in", 
...
#levels is number of categories
#labels are the names of categories 
...
Surface_Temp_Day <- PeterPaul.chem.nutrients %>%
  filter(depth == 0) %>%
  group_by(lakename)

#alpha sets transparency
#facet_wrap(vars()) set apart data
Surface_Temp_Day_plot <- Surface_Temp_Day %>%
  ggplot(aes(
      x=daynum,
      y=temperature_C,
      color= year4)) + 
  ...
  labs(
    title = "Surface Temperature Vs. Day of Year",
    x= "Day Number",
    y= "Surface Temperature in Celcius",
    color= "Year") +
  scale_color_viridis(, direction = -1)
#view plot
Surface_Temp_Day_plot  
#2. 
