...
| Metadata Attribute Item Name | Metadata Values                     |
|-----------------------------:|:------------------------------------|
...
#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr); library(cowplot)
here()
...
```{r, echo=FALSE, }
Aileentheme <- theme_classic(base_size = 16) +
...
Ozone2018 <- ggplot(Ozone2018, aes((x=factor(Month, levels=1:12,
...
Ozone2019 <- ggplot(Ozone2019, aes((x=factor(Month, levels=1:12,
  labels=month.abb)), y=Ozone)) +
  geom_boxplot() +
  labs(
    x= "Month",
...
```{r, echo=FALSE, fig.cap="Mean Ozone Levels", fig.height=10.5, fig.width=12, message=FALSE, warning=FALSE, align="left"}
#combine plots
plot_grid(Ozone2018, Ozone2019, nrow=1, labels=c(''))
```
...
<https://github.com/AileenLavelle/EDA-Spring2023.git>
