...
```{r}
#12
GaringerOzone.monthlytrend <- mk.test(GaringerOzone.monthly.ts)
GaringerOzone.monthlytrend
summary(GaringerOzone.monthlytrend)
```

> Answer: Mann-Kendall is used in this case because it assumes no seasonality, allows for missing data and is non-parametric. 
...
> Answer: The summer and spring months have the highest average monthly mean ozone concentrations. Throughout the study period we can see an overall trend of a slight decreasing ozone concentrations, in all months S = -7.520000e+02 (p-value = 0.088). 
...
GaringerOzone.monthly_noseason <- mutate(GaringerOzone.monthly_noseason,
        Observed = GaringerOzone.monthly$meanOzone,     
        Date = GaringerOzone.monthly$Date)

#16
firstmonth <- month(first(GaringerOzone.monthly$Date))
firstyear <- year(first(GaringerOzone.monthly$Date))

GaringerOzone.monthlycomponents.ts <- ts(GaringerOzone.monthly_noseason$trend,
                   start=c(f_year,f_month),
                   frequency=12) 

mk.test(GaringerOzone.monthly_noseason)
```
