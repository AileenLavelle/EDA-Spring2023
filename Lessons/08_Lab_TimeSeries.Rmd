...
summary(BRcomplete$Rainfall)
summary(BRcomplete$Temperature_C)

BR_complete <- BR_complete %>%
  drop_na(Rainfall)
...
tempovertime <- ggplot(BR_complete, aes(x = Date, y = Temperature_C)) +
  geom_point() +
  geom_line() +
...
Rainfallovertime <- ggplot(BR_complete, aes(x = Date, y = Rainfall)) +
  geom_point() +
  geom_line() +
  geom_smooth( method = lm ) +
  ylab("Rainfall (cm)") +
...
print(Rainfallovertime)
#temperature increases over time and rainfall stays stable.
#rainfall has seasonality because there is an up and down movement on the graph.
#temperature also has seasonality.
```
...
BR_Temperature_ts <- ts(BR_complete$Temperature_C, start = c(f_year,f_month), frequency = 12)
```
...
#the larger the bar on the right hand side the smaller the impact on the data variation.
...
ggplot(BR_Rainfall_Components) +
  ...
  #geom_line(aes(y = seasonal), col="green") +
  geom_hline(yintercept = 50, lty = 2) +
  ylab(expression("Rainfall (cm)"))
...
ggplot(BR_Temperature_Components) +
  ...
  ylab(expression("Temperature (Celcius)"))
...
Temp_trend <- trend :: smk.test(BR_Temperature_ts)
summary(Temp_trend)

Rain_trend <- trend :: smk.test(BR_Rainfall_ts)
summary(Temp_trend)
